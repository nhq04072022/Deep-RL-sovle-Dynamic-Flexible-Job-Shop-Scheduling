...
class creation:
    ...
    def output(self):
        job_info = [[i, self.sequence_list[i], self.pt_list[i], self.create_time[i], self.due_list[i]] for i in range(self.index_jobs)]
        job_info = [[list(item) if isinstance(item, np.ndarray) else item for item in row] for row in job_info]
