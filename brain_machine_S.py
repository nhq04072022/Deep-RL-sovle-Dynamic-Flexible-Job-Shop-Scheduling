...
class sequencing_brain:
    ...
    def state_multi_channel(self, sqc_data):
        # === 1. Thông tin về hệ thống và hàng đợi tại máy hiện tại ===
        ...
        if arriving_job_no > 0:
            ...
            arriving_job_time = 0.0
            arriving_job_slack = 0.0

        # === 2. Thông tin toàn hệ thống ===
        ...
        # === 3. Hệ số biến thiên thời gian xử lý còn lại trong hệ thống ===
        rem_pt = [x.tolist() for m in self.m_list for x in m.remaining_pt_list]
        rem_pt_flat = [item for sublist in rem_pt for item in sublist]
        rem_pt_mean = np.mean(rem_pt_flat) if len(rem_pt_flat) > 0 else 1e-6
        global_pt_CV = np.std(rem_pt_flat) / rem_pt_mean if rem_pt_mean > 1e-6 else 0

        # === 4. Hàng đợi tại máy hiện tại ===
        local_pt = sqc_data[0]
        local_remaining_pt = sqc_data[1]

        local_pt_mean = np.mean(local_pt) if len(local_pt) > 0 else 1e-6
        local_remaining_pt_mean = np.mean(local_remaining_pt) if len(local_remaining_pt) > 0 else 1e-6

        local_pt_sum = np.sum(local_pt)
        local_pt_min = np.min(local_pt) if len(local_pt) > 0 else 0
        local_pt_CV = np.std(local_pt) / local_pt_mean if local_pt_mean > 1e-6 else 0

        local_remaining_pt_sum = np.sum(local_remaining_pt)
        local_remaining_pt_max = np.max(local_remaining_pt) if len(local_remaining_pt) > 0 else 0
        local_remaining_pt_CV = np.std(local_remaining_pt) / local_remaining_pt_mean if local_remaining_pt_mean > 1e-6 else 0

        avlm = sqc_data[8]
        avlm_mean = np.mean(avlm) if len(avlm) > 0 else 1e-6
        avlm_min = np.min(avlm) if len(avlm) > 0 else 0
        avlm_CV = np.std(avlm) / avlm_mean if avlm_mean > 1e-6 else 0

        # === 5. Thời gian đến hạn và slack ===
        time_till_due = sqc_data[5]
        ttd_mean = np.mean(time_till_due) if len(time_till_due) > 0 else 1e-6
        ttd_min = np.min(time_till_due) if len(time_till_due) > 0 else 0
        ttd_CV = np.std(time_till_due) / ttd_mean if ttd_mean > 1e-6 else 0
        ttd_CV = np.clip(ttd_CV, -2, 2)

        realized_tard_rate = (time_till_due < 0).sum() / local_job_no if local_job_no > 0 else 0

        slack = sqc_data[6]
        slack_mean = np.mean(slack) if len(slack) > 0 else 1e-6
        slack_min = np.min(slack) if len(slack) > 0 else 0
        slack_CV = np.std(slack) / slack_mean if slack_mean > 1e-6 else 0
        slack_CV = np.clip(slack_CV, -2, 2)

        exp_tard_rate = (slack < 0).sum() / local_job_no if local_job_no > 0 else 0

        # === 6. Phân phối thời gian khả dụng giữa các máy ===
        pt_share = available_time[sqc_data[-1]] / (np.sum(available_time) + 1e-6)

        # === 7. Tổng hợp vector trạng thái ===
        ...
        s_t = np.nan_to_num(np.concatenate([
            no_info, pt_info, remaining_pt_info,
            ttd_slack_info, progression, heterogeneity
        ]), nan=0.0, posinf=1.0, neginf=-1.0)

        return torch.tensor(s_t, dtype=torch.float)
